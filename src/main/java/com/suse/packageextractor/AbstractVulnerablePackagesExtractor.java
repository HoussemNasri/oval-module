...
import com.suse.ovaltypes.BaseCriteria;
import com.suse.ovaltypes.CriteriaType;
import com.suse.ovaltypes.CriterionType;
 ...
 * The extraction process depends strongly on the way the vulnerability criteria tree is structured. For that reason,
 * we have multiple implementations of the extractor for each Linux distribution.
 */
public abstract class AbstractVulnerablePackagesExtractor {
    ...
    protected AbstractVulnerablePackagesExtractor(DefinitionType vulnerabilityDefinition, OvalObjectManager ovalObjectManager, OvalTestManager ovalTestManager, OvalStateManager ovalStateManager) {
    ...
    private void collectCriterionsHelper(BaseCriteria criteria, int currentLevel, int maxNestingLevel, List<CriterionType> criterions) {
